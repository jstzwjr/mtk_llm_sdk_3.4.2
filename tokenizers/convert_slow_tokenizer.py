# Copyright 2018 The HuggingFace Inc. team.  # noqa: CPY001
#
...
def import_protobuf(_):
    """Import sentencepiece protobuf."""
...
def generate_merges(vocab, vocab_scores):
    """Generate merges from vocab and vocab scores."""
    ...
    return [(val[0], val[1]) for val in merges]
...
class SentencePieceExtractor:
    ...
    def __init__(self, model: str):
        """Initialize SentencePieceExtractor."""
    ...
    def extract(self, vocab_scores=None) -> Tuple[Dict[str, int], List[Tuple]]:
        """Extract vocab and merges from vocab scores.
...
class GemmaSentencePieceExtractor(SentencePieceExtractor):
    """Extractor implementation for SentencePiece trained models for Gemma."""
...
class BertConverter(Converter):
    """Bert Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Bert tokenizer."""
...
class SplinterConverter(Converter):
    """Splinter Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Splinter tokenizer."""
        vocab = self.original_tokenizer.vocab
        tokenizer = Tokenizer(WordPiece(vocab, unk_token=str(self.original_tokenizer.unk_token)))
...
class FunnelConverter(Converter):
    """Funnel Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Funnel tokenizer."""
        vocab = self.original_tokenizer.vocab
        tokenizer = Tokenizer(WordPiece(vocab, unk_token=str(self.original_tokenizer.unk_token)))
...
class MPNetConverter(Converter):
    """MPNet Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the MPNet tokenizer."""
        vocab = self.original_tokenizer.vocab
        tokenizer = Tokenizer(WordPiece(vocab, unk_token=str(self.original_tokenizer.unk_token)))
...
class OpenAIGPTConverter(Converter):
    """OpenAIGPT Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the OpenAIGPT tokenizer."""
...
class HerbertConverter(Converter):
    """Herbert Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Herbert tokenizer."""
...
class Qwen2Converter(Converter):
    """Qwen2 Converter class."""

    def converted(
        self, vocab: Optional[Dict[str, int]] = None, merges: Optional[List[Tuple[str, str]]] = None
    ) -> Tokenizer:
        """Gets the Qwen2 tokenizer."""
        if not vocab:
            vocab = self.original_tokenizer.encoder
        if not merges:
            merges = list(self.original_tokenizer.bpe_ranks.keys())
...
class RobertaConverter(Converter):
    """Roberta Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Roberta tokenizer."""
...
class DebertaConverter(Converter):
    """Deberta Converter class."""

    def converted(self) -> Tokenizer:
        """Gets the Deberta tokenizer."""
        ot = self.original_tokenizer
        vocab = ot.encoder
        merges = list(ot.bpe_ranks.keys())
...
class SpmConverter(Converter):
    """Sentencepiecemodel Converter class."""
    ...
    def __init__(self, *args):
        """Initialize the Sentencepiecemodel Converter."""
        super().__init__(*args)
    ...
    def unk_id(self, proto):
        """Gets the SPM unk token id."""
        return proto.trainer_spec.unk_id

    def tokenizer(self, proto):
        """Gets the SPM tokenizer."""
        ...
        spm_added_tokens = [
            (id_, p.piece, p.type == 3 or p.piece in self.special_tokens)
            for id_, p in enumerate(proto.pieces)
            if p.type in [3, 4]
        ]
    ...
    def normalizer(self, proto):
        ...
        if not precompiled_charsmap:
            return normalizers.Sequence(normalizers_)
        return normalizers.Sequence([normalizers.Precompiled(precompiled_charsmap), *normalizers_])
...
class AlbertConverter(SpmConverter):
    ...
    def normalizer(self, proto):
        ...
        if not self.original_tokenizer.keep_accents:
            listnormalizers_.append(normalizers.NFKD())
            listnormalizers_.append(normalizers.StripAccents())
        if self.original_tokenizer.do_lower_case:
            listnormalizers_.append(normalizers.Lowercase())

        precompiled_charsmap = proto.normalizer_spec.precompiled_charsmap

        if precompiled_charsmap:
            listnormalizers_.append(normalizers.Precompiled(precompiled_charsmap))

        listnormalizers_.append(normalizers.Replace(Regex(' {2,}'), ' '))
        return normalizers.Sequence(listnormalizers_)
...
class DebertaV2Converter(SpmConverter):
    ...
    def normalizer(self, proto):
        ...
        if self.original_tokenizer.do_lower_case:
            listnormalizers_.append(normalizers.Lowercase())
        listnormalizers_.append(normalizers.Strip())
...
class ReformerConverter(SpmConverter):
    """Reformer Converter class."""


class RemBertConverter(SpmConverter):
    """RemBert Converter class."""
...
class BertGenerationConverter(SpmConverter):
    """BertGeneration Converter class."""


class PegasusConverter(SpmConverter):
    """Pegasus Converter class."""
...
class T5Converter(SpmConverter):
    ...
    def vocab(self, proto):
        """Gets the vocab."""
        num_extra_ids = self.original_tokenizer._extra_ids  # noqa: SLF001
...
class TikTokenConverter:
    ...
    def extract_vocab_merges_from_model(self, tiktoken_url: str):
        """Extract vocab merges from model."""
        try:
...
def convert_slow_tokenizer(transformer_tokenizer, from_tiktoken=False) -> Tokenizer:
    ...
    Args:
        ...
        from_tiktoken (bool, optional): Whether to use the `tiktoken` library to convert the tokenizer instead of
            sentencepiece. Defaults to False.
    ...
    try:
        print('Converting from Tiktoken')
