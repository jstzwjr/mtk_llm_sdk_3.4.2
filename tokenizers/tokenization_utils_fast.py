# Copyright 2020 The HuggingFace Inc. team.  # noqa: CPY001
#
...
class PreTrainedTokenizerFast(PreTrainedTokenizerBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        truncation_ = self._tokenizer.truncation

        if truncation_ is not None:
            self._tokenizer.enable_truncation(**truncation_)
            kwargs.setdefault('max_length', truncation_['max_length'])
            kwargs.setdefault('truncation_side', truncation_['direction'])
            kwargs.setdefault('stride', truncation_['stride'])
            kwargs.setdefault('truncation_strategy', truncation_['strategy'])
        else:
            self._tokenizer.no_truncation()

        padding_ = self._tokenizer.padding
        if padding_ is not None:
            self._tokenizer.enable_padding(**padding_)
            kwargs.setdefault('pad_token', padding_['pad_token'])
            kwargs.setdefault('pad_token_type_id', padding_['pad_type_id'])
            kwargs.setdefault('padding_side', padding_['direction'])
            kwargs.setdefault('max_length', padding_['length'])
            kwargs.setdefault('pad_to_multiple_of', padding_['pad_to_multiple_of'])
    ...
    def is_fast(self) -> bool:
        """Is fast tokenizer property."""
        return True

    @property
    def can_save_slow_tokenizer(self) -> bool:
        """`bool`: Whether or not the slow tokenizer can be saved.

        Usually for sentencepiece based slow tokenizer, this
    ...
    def added_tokens_encoder(self) -> Dict[str, int]:
        ...
        The added tokens encoder is cached for performance
    ...
    def set_truncation_and_padding(
        ...
        if truncation_strategy == TruncationStrategy.DO_NOT_TRUNCATE:
            ...
            # truncation_ might contain more keys that the target `transformers`
            ...
            # targets.
            current = None if truncation_ is None else {k: truncation_.get(k, None) for k in target}
    ...
    def train_new_from_iterator(
        ...
        **kwargs,
        ...
        if (tokenizer_json['pre_tokenizer'] is not None and tokenizer_json['pre_tokenizer']['type'] == 'ByteLevel') or (
