...
blank_str = r'<\|blank_([\d]+)\|>'
blank_pattern = re.compile(blank_str)


class Tokenizer:
    """Tokenizer class."""

    def __init__(self, model_path: str, max_blank_length=50, use_lb=True, fix_zh=False, **kwargs):
        ...
        self.vocab = {self._convert_id_to_token(i): i for i in range(self.sp_model.vocab_size())}
        ...
        self.special_tokens = ['<|tab|>', p, '[EOS]', '<|lb|>'] + [f'<|unused{i}|>' for i in range(10)]
        self.blank_tokens = [self.get_blank_token(i) for i in range(self.max_blank_length)]
        zh_char_list = ['，', '；', '！', '？', '：', '（', '）', '￥', '…']  # noqa: RUF001
        self.zh_char = {f'<|unused{i}|>': zh_char_list[i] for i in range(9)}
        logger.info(f'#words: {self.sp_model.vocab_size()} - BOS ID: {self.bos_id} - EOS ID: {self.eos_id}')

    def preprocess_zhchar(self, x: str):
        """Preprocess Chinese chars."""
    ...
    def _preprocess(self, x: str):
        ...
        return x.replace('\t', '<|tab|>').replace('\n', self.lb_token).replace('<|rag|>', '<|unused9|>')

    def _postprocess(self, x: str):
        text = x.replace('<|tab|>', '\t').replace(self.lb_token, '\n').replace('<|unused9|>', '<|rag|>')
        text = text.replace(self.pad_token, '')
        text = re.sub(blank_pattern, lambda m: ' ' * int(m.group(1)), text)
        if self.fix_zh:
...
class MiTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the MiTokenizer class."""
        self.kwargs = {
            'bos_token': bos_token,
            'eos_token': eos_token,
            'end_token': end_token,
        ...
        self.pad_token = '<|pad|>'
        self.split_by_special_token = kwargs.get('split_by_special_token', False)

        self.blank_tokens = [self.tokenizer.get_blank_token(i) for i in range(self.tokenizer.max_blank_length)]
    ...
    def preprocess_text(self, inputs):
        """Preprocess function."""
        return self.tokenizer._preprocess(inputs)  # noqa: SLF001

    def _tokenize(self, text, **kwargs):
        """Returns a tokenized string."""
        text = self.preprocess_text(text)
        return self.tokenizer._tokenize(text)  # noqa: SLF001

    def tokenize(self, text, **kwargs):
        """Tokenize function."""
        if self.split_by_special_token:
