...
# ==============================================================================
"""Define Qwen2Audio projector class."""
...
from .configuration_projector_qwen2_audio import Qwen2AudioProjectorConfig


class Qwen2AudioProjector(BaseProjector):
    """Qwen2Audio Projector class.

    This class implements the projector FC.
    """

    def __init__(self, config: Qwen2AudioProjectorConfig, dtype=torch.float32, jit_trace=False):
        """Initialize Qwen2AudioProjector.
        ...
        self.projector = nn.Linear(self.d_model, self.hidden_size)

    def _generate_default_state_dict_mapping(self):
        self.state_dict_mapping = {
            '0.weight': {'projector.weight': 'multi_modal_projector.linear.weight'},
            '0.bias': {'projector.bias': 'multi_modal_projector.linear.bias'},
        }
    ...
    def load_weights(self, state_dict):
        ...
        """
        logger.debug('Enter Qwen2Audio projector load_weights')
    ...
    def forward(self, audio_embedding):
        ...
        Args:
            audio_embedding (torch.Tensor): The output image embeddings from vision encoder.

        Returns:
            Tuple[torch.Tensor, Tuple[torch.Tensor]]: The hidden states and the projector outputs.
        """
        return self.projector(audio_embedding.to(self.device_list[0]))
    ...
    def get_ptq_inputs(self, args, **kwargs):
        ...
        """
        input_shapes = [[1, int(self.max_source_positions / 2), self.d_model]]
        ...
        if args.calibration_dataset == 'fake':
            ...
            def calib_data_gen():
                for _ in range(10):
                    yield [np.random.rand(1, int(self.max_source_positions / 2), self.d_model).astype(np.float32)]
        else:
