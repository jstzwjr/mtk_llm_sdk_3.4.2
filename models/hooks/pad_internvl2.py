...
class PadInternVL2(NumpyToTorch):
    """PadInternVL2 that extend NumpyToTorch class.
    ...
    def forward(self, inp, **kwargs):
        ...
        if pipeline_type == 'float':
            ...
            kwargs.update({'internvl2_original_batch_size': batch_size})
            return out, kwargs

        arr = out['input_features'] if batch_feature else out
        max_patch = kwargs.get('internvl2_max_patch', None)
        batch_size = arr.shape[0]
        kwargs.update({'internvl2_original_batch_size': batch_size})
        if max_patch is None:
            logger.error('max_patch must be passed when inference quantized InternVL2.')
        batch_pad = (max_patch + 1) - batch_size
        if batch_size < (max_patch + 1):  # Need to pad Batch dimenstion to 13
