    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if is_batched_numpy and len(raw_speech.shape) > 2:
            logger.error(f'Only mono-channel audio is supported for input to {self}', err=ValueError)
        ...
        if return_attention_mask:
            ...
            padded_inputs['attention_mask_audio'] = padded_inputs['attention_mask'][:, :: self.hop_length]
            padded_inputs.pop('attention_mask')  # change name to prevent conflict with vision
            ...
            padded_inputs['attention_mask_audio'] = np.reshape(padding_mask * -200.0, (batch_size, 1, max_seq_len))
